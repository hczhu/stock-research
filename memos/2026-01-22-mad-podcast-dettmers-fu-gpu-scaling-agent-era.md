...
- ## Stock Read-Throughs
	- **[[$NVDA]] — structurally positive demand, more nuanced moat**
		- Positive: Blackwell-era models had barely reached users at the interview date; tens-of-thousands-chip clusters and underutilized systems imply substantial latent capability and ongoing demand for CUDA, NVLink, networking, and rack-scale systems.
		- Positive: mega-kernels show that programmable GPUs can approximate some benefits of dedicated accelerators, preserving flexibility as architectures change.
		- Pressure: better utilization reduces chips required per fixed workload, and inference is more open to alternative silicon than training.
		- Key question: does [[$NVDA]] capture optimization value through higher system performance and platform pricing, or do customers retain most savings and slow unit growth?
	- **[[$AMD]] — credible inference opening, software remains decisive**
		- Hardware specifications alone are insufficient; Fu emphasizes that AMD GPUs require different software abstractions. Ecosystem investment such as HipKittens is a prerequisite for competitive utilization.
		- Inference diversification and customer desire for a second source create an opening, but production reliability, kernels, libraries, and developer support determine conversion.
	- **[[$TSM]] — beneficiary of heterogeneity and system complexity**
		- A multi-chip market can expand the number of leading-edge designs even if no competitor displaces [[$NVDA]]. Advanced nodes and packaging remain common dependencies across GPUs, ASICs, and edge accelerators.
		- Slower performance scaling may push designers toward larger systems, chiplets, memory integration, and packaging intensity.
	- **[[$MU]] and HBM suppliers — bandwidth scarcity versus efficiency**
		- Memory movement remains fundamental, and HBM manufacturing complexity limits rapid supply response.
		- The counterforce is software: quantization, smaller models, mega-kernels, and speculative decoding reduce bytes moved per completed task. Demand remains bullish only if task volume and model intensity outgrow these savings.
	- **[[$MSFT]] and developer platforms — distribution advantage with application-layer disruption**
		- Agent adoption raises cloud inference, code-hosting, and tool-use demand.
		- The same agents lower the cost of reproducing software features, pressuring seat-based SaaS and increasing the value of proprietary workflow data, governance, and system-of-record control.
	- **Private inference platforms such as Together AI — technically valuable, competitively exposed**
		- Large utilization gaps create room for differentiated kernels and serving software.
		- However, speedups can diffuse into open source or be incorporated by model labs and hyperscalers. Durable value requires continuous optimization, customer traffic, distribution, and production reliability.

- ## Scenario Framework
	- | Scenario | Technical path | Market consequence | Likely beneficiaries |
	  |---|---|---|---|
	  | Effective-compute boom | New clusters, kernels, post-training, and architecture gains produce a large fraction of Fu's 100x headroom | Capability and agent use accelerate; efficiency is overwhelmed by demand | [[$NVDA]], [[$TSM]], HBM suppliers, clouds, inference platforms, developer tools |
	  | Efficiency-led digestion | Models improve, but utilization and smaller models reduce hardware per task faster than usage grows | Accelerator unit growth slows; value shifts to software and distribution | Hyperscalers, application platforms, [[$AMD]] as lower-cost alternative, edge ecosystems |
	  | Physical-wall case | HBM, power, networking, data, and diminishing returns bind; frontier gains disappoint | Capex returns fall and model differentiation compresses | Small-model vendors, automation software, customers using cheaper open models |
	  | Agent diffusion lag | Models are capable but enterprises cannot redesign workflows, govern access, or verify output | Token growth trails technical capability and SaaS disruption takes longer | Incumbent systems of record, observability, security, services |

- ## Key Tests for the Thesis
	- **Blackwell model evidence**: capability, training speed, and economics from models genuinely pre-trained on GB200/NVL72-class systems rather than fine-tuned on them.
	- **Utilization progression**: realized training MFU and inference utilization on production workloads, not vendor peak FLOPS.
	- **Price-elasticity evidence**: whether lower token prices create more total inference revenue, tokens, agent steps, and complex tasks.
	- **Enterprise small-model adoption**: private-repository tuning, on-premise deployment, and workloads migrating away from frontier APIs.
	- **Hardware share at inference**: sustained production wins for [[$AMD]], TPUs, or specialized accelerators versus evaluation announcements.
	- **Agent productivity outside elite users**: time-to-completion, defect rates, review burden, and adoption by average employees rather than kernel experts.
	- **Software labor response**: whether companies reduce headcount, raise output targets, or expand previously uneconomic software projects.
	- **HBM intensity per useful task**: model/context growth and agent concurrency versus gains from quantization, sparsity, caching, and kernel fusion.
