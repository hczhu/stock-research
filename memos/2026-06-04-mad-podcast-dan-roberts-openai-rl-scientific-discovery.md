...
- ## Stock Read-Throughs
	- **[[$NVDA]] — RL and scientific agents expand the inference surface**
		- Reasoning RL requires many model-generated trajectories during training, while deployment can spend hours on a single high-value problem.
		- Counterexample search, parallel sampling, verifier models, and repeated refinement can raise compute per successful task even as individual tokens become cheaper.
		- The bear case is rapid improvement in reward density, search efficiency, or custom accelerators. Demand depends on task-volume elasticity exceeding efficiency gains.
	- **[[$MSFT]] — OpenAI exposure plus enterprise workflow distribution**
		- If OpenAI converts scientific reasoning into products, Microsoft can benefit through model distribution and Azure infrastructure without requiring fully autonomous science.
		- GitHub, enterprise identity, data, and developer tooling provide environments where outcomes can be tested and feedback collected.
		- The major uncertainties are the economics and durability of the OpenAI relationship, cloud gross margins under inference-heavy workloads, and competition from other labs.
	- **[[$GOOGL]] — formal verification is a strategic differentiator**
		- DeepMind's machine-checkable proof workflow is well suited to domains where trust matters as much as discovery speed.
		- Google's internal model research, TPUs, cloud, search, and scientific teams create a vertically integrated path from training to application.
		- Natural-language systems may explore more broadly, so formalization must avoid becoming a costly bottleneck.
	- **[[$AMZN]] and [[$ORCL]] — infrastructure exposure without a single-model bet**
		- Bursty RL rollouts, scientific simulation, storage, and long-running inference can expand cloud consumption across model providers and enterprise customers.
		- Value capture depends on winning scarce accelerator capacity and differentiated workloads rather than competing only on commodity compute pricing.
	- **Scientific and enterprise software — verification is more durable than a thin interface**
		- Platforms controlling domain data, experimental workflows, simulation, audit trails, or trusted expert review can turn general models into reliable products.
		- A wrapper that merely submits prompts is exposed as base models improve. The defensible layer defines the problem, supplies proprietary context, and proves that the answer worked.

- ## Scenario Framework
	- | Scenario | Technical path | Economic outcome | Equity read-through |
	  |---|---|---|---|
	  | RL discovery scales | Models repeatedly generate validated math, code, and scientific results | Long rollouts and verification become a new high-value compute category | Bullish for [[$NVDA]], cloud platforms, and owners of scientific environments |
	  | Human-in-the-loop science | Models generate candidates but experts retain question selection and validation | Adoption grows steadily; value concentrates in workflow and verification layers | Positive for infrastructure, mixed for labs, favorable to domain incumbents with trusted data |
	  | Reward bottleneck | Results outside narrow verifiable domains remain unreliable or reward-hacked | Scientific autonomy stalls and compute ROI disappoints | Pressures frontier-lab economics; supports audit, eval, and human-review vendors |
	  | Efficiency outruns demand | Better algorithms sharply reduce rollout and test-time compute per success | Capability expands while infrastructure intensity falls | Benefits application margins; weakens the accelerator-demand thesis |

- ## Key Tests for the Thesis
	- **Repeatability**: additional peer-validated discoveries from models over the six months following the episode.
	- **Domain breadth**: movement from mathematics and code into physics, chemistry, biology, and professional workflows with measurable outcomes.
	- **Autonomy depth**: how much of question selection, experiment design, execution, verification, and interpretation models complete without intervention.
	- **Inference intensity**: reasoning time, parallel samples, tool calls, and verifier usage per successful high-value task.
	- **RL efficiency**: reward information per rollout, success rate, and compute required for a given capability improvement.
	- **Formal verification adoption**: whether natural-language discoveries are routinely converted into machine-checkable proofs or executable tests.
	- **Commercial evidence**: paid scientific products, enterprise renewal, workflow retention, and customer willingness to fund long-running tasks.
	- **AI-research feedback loop**: credible disclosures that model-generated code, experiments, or hypotheses materially shorten frontier-model development cycles.

- ## Key Takeaways
	- The episode's strongest claim is not that AI has become an autonomous scientist. It is that RL-trained reasoning models have crossed from recombining known answers into at least some genuine exploration and original discovery.
	- The investable mechanism is compute amplification: pre-training creates a knowledgeable prior, RL teaches search, and test-time inference spends variable compute until a valuable result is found and verified.
	- This supports accelerator and cloud demand if the volume and ambition of delegated tasks grow faster than algorithms become efficient. It also creates a distinct opportunity for formal verification, simulations, evals, and vertical data owners.
	- The gating variable is trustworthy output. Research taste, objective design, and validation remain human-heavy, so the near-term commercial model is augmented discovery rather than unattended science.
