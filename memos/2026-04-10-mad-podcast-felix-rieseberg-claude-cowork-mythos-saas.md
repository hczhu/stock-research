...
- ## Stock Read-Throughs
	- **[[$AMZN]] and [[$GOOGL]] — Anthropic growth pulls through cloud consumption**
		- Longer agent horizons, many prototypes, computer use, and managed third-party agents increase inference and execution workloads.
		- Cloud partners can win regulated deployments through data residency, security, and procurement even when Anthropic owns the user experience.
		- Risk: Anthropic's vertical expansion captures the highest-value product layer and reduces cloud differentiation to infrastructure economics.
	- **[[$MSFT]] — control points in OS, identity, GitHub, and productivity**
		- Local agents make Windows, browser sessions, enterprise identity, and endpoint management strategically important.
		- GitHub and Office distribution provide context and workflow surfaces, but external agents can weaken application-level lock-in if files and actions are portable.
	- **[[$CRM]], [[$NOW]], [[$TEAM]], and [[$GTLB]] — system-of-record defense versus interface compression**
		- Durable case: authoritative data, permissions, workflows, audit history, and embedded distribution become the substrate agents use.
		- Bear case: Cowork-like agents perform the user's goal across files and applications, reducing direct seat engagement and making fixed interfaces less valuable.
		- Key response is to expose reliable actions and context to agents while retaining governance and transaction ownership.
	- **[[$CRWD]] and [[$PANW]] — larger automated attack and defense market**
		- Mythos-like capabilities increase vulnerability discovery and remediation speed.
		- They also create new risks from autonomous exploitation, sandbox escape, credential access, and prompt injection.
		- Security platforms need endpoint, identity, network, code, and agent telemetry to verify what autonomous systems actually did.
	- **AI infrastructure — usage can outgrow software efficiency**
		- Ten prototypes instead of one, unattended recurring tasks, week-long agents, and 200-page outputs expand total inference.
		- Better models may reduce retries and code overhead per task; aggregate demand depends on whether new tasks grow faster than efficiency.

- ## Scenario Framework
	- | Scenario | Path | Industry consequence | Relative beneficiaries |
	  |---|---|---|---|
	  | Agent-platform concentration | Anthropic absorbs runtime, memory, tools, and managed deployment | Application builders depend on a few model platforms | Anthropic ecosystem and cloud partners; pressure on generic agent infrastructure |
	  | SaaS becomes agent-accessed systems of record | External agents own the interface while incumbents retain data and transactions | Seat engagement falls but core platforms remain essential | Strong data/governance incumbents; agent UX leaders |
	  | Foundation models absorb applications | Model can construct task-specific software and memory on demand | Thin vertical wrappers lose pricing and differentiation | Model providers, clouds, users; pressure on narrow SaaS |
	  | Workflow specialists win | Diffusion is slow and domain/regulatory complexity persists | Vertical products package trust, policy, and accountability | Regulated vertical SaaS, services, security, systems integrators |
	  | Cybersecurity discontinuity | Offensive and defensive models automate vulnerability discovery | Security spending rises but tool categories consolidate | Broad telemetry and response platforms, secure agent runtimes |
	  | Local-agent standard | Authenticated local context proves safer and more useful than cloud replication | Endpoint and OS control become more valuable | Desktop platforms, identity, endpoint security, browsers |

- ## Key Tests for the Thesis
	- **Cowork usage**: active users, task duration, scheduled-task frequency, files touched, and repeat delegation beyond coding.
	- **Enterprise readiness**: regulated-industry availability, third-party cloud deployment, audit logs, data residency, identity, and administrative controls.
	- **Agent economics**: tokens and execution cost per completed workflow versus human time saved.
	- **SaaS displacement**: seat contraction, lower direct engagement, and workflows initiated by external agents rather than native interfaces.
	- **System-of-record durability**: whether agents continue to query and update incumbent platforms or bypass them with files and bespoke databases.
	- **Security performance**: vulnerabilities found, remediation time, false positives, escape rate, and incident volume involving agents.
	- **Local versus cloud execution**: user trust, latency, enterprise policy, and the share of computer-use sessions running on endpoints.
	- **Prototype-to-revenue conversion**: whether dramatically higher experiment counts produce more successful products or simply more internal noise.
	- **Model overhang**: frequency with which a product update unlocks already-existing capability without a new model release.
