...
- ## Stock Read-Throughs
	- **[[$V]] and [[$MA]] — agent commerce expands transactions but challenges card economics**
		- More autonomous purchases and embedded checkout can increase payment volume.
		- Shared tokens can preserve cards as a funding source while hiding the credential from the agent.
		- One-cent machine payments require different fees and settlement, giving stablecoins an opening.
	- **[[$PYPL]] — wallet distribution is strategically valuable**
		- Consumer trust, stored funding methods, purchase protection, and merchant reach are strong starting assets.
		- Link's repositioning shows that wallets must become programmable agent-control surfaces, not only faster human checkout.
	- **[[$COIN]] and [[$CRCL]] — stablecoin utility moves from trading toward machine commerce**
		- Streaming settlement, programmable wallets, and micropayments fit agents structurally.
		- The risk is that economics accrue to Stripe, wallets, or chains while stablecoin issuers capture only reserve income.
	- **[[$SHOP]], [[$WIX]], and [[$BIGC]] — agent discoverability becomes a distribution requirement**
		- ACP can expose merchant catalogs across AI surfaces without separate integrations.
		- Platforms retain value through inventory, checkout, fulfillment, returns, identity, and merchant relationships even if the storefront UI loses traffic.
		- AI surfaces can become new gatekeepers and demand economics or own the customer relationship.
	- **[[$GOOGL]], [[$MSFT]], and [[$META]] — AI surfaces become commerce channels**
		- Embedded buy buttons convert AI discovery into transaction revenue and richer intent data.
		- Neutral protocols broaden supply, but these platforms may favor their own advertising, payment, or merchant economics.
	- **[[$AFRM]] — delegated payments can broaden BNPL distribution**
		- Shared payment tokens can represent BNPL alongside cards, allowing agents to choose funding under user policy.
		- Automated credit selection raises suitability, disclosure, and consumer-protection questions.
	- **Fraud and billing software — AI gross margin requires lifecycle controls**
		- Signup identity, usage behavior, token metering, streaming payment, revenue recognition, and anomaly detection converge.
		- Point solutions face pressure from integrated networks that see both the buyer and seller across the entire transaction lifecycle.

- ## Scenario Framework
	- | Scenario | Adoption path | Stripe or ecosystem result | Public-equity read-through |
	  |---|---|---|---|
	  | Embedded-checkout growth | AI recommends; humans approve purchases | Higher conversion and payment volume without full autonomy | Positive for commerce platforms, card networks, and AI surfaces |
	  | Agent micro-firm emergence | Agents buy, provision, integrate, sell, and collect revenue | New company formation and machine payments accelerate | Benefits payments, cloud, developer tools, and stablecoin infrastructure |
	  | Trust plateau | Consumers refuse high-value delegation | Agent commerce remains a checkout feature, not a new economy | Limits wallet and protocol upside; incumbents retain interfaces |
	  | Token-fraud escalation | Abuse grows faster than lifecycle defenses | PLG funnels close, gross margins deteriorate, enterprise sales dominate | Benefits integrated fraud platforms; hurts self-serve AI vendors |
	  | Open-protocol commoditization | ACP and MPP become common standards | Transaction volume expands but orchestration take rates stay low | Benefits merchants and agents; reduces control-plane monetization |
	  | Stablecoin machine rail | Streaming micropayments scale with low cost and instant settlement | New payment categories become viable | Positive for [[$COIN]] and [[$CRCL]]; forces [[$V]] and [[$MA]] adaptation |

- ## Key Tests for the Thesis
	- **Autonomy progression**: share of purchases completed without item-level human approval and average order value by autonomy level.
	- **Protocol adoption**: active ACP and MPP merchants, platforms, agents, catalogs, and completed transactions.
	- **Link agent usage**: authorized wallets, agent spend, retention, revocation, and fraud versus human checkout.
	- **Token pricing**: mix of subscription, prepaid credits, usage overage, and true streaming billing across AI companies.
	- **Fraud loss**: multi-account incidence, free-trial abuse, unpaid usage, resale, and recovered COGS.
	- **Network advantage**: incremental Radar accuracy from buyer-plus-agent identity across Stripe's AI ecosystem.
	- **Stablecoin settlement**: machine-payment volume, average transaction value, cost, latency, failed payments, and dispute mechanisms.
	- **Vibe deployment**: agent-provisioned services and conversion from generated application to first revenue.
	- **Startup dynamism**: business formations, survival, first-dollar timing, revenue per employee, and international mix.
	- **Liability framework**: allocation of responsibility among user, agent developer, model provider, merchant, wallet, and payment processor.

- ## Key Takeaways
	- Agentic commerce is developing first as an embedded distribution and checkout channel, while fully autonomous commerce remains constrained by trust and liability.
	- Stripe's strongest position is the intersection of authorization, billing, fraud, wallets, and merchant data. A protocol alone is reproducible; a dense network of known buyers, agents, and sellers is harder to copy.
	- AI's real marginal cost changes software finance. Usage-based pricing, real-time metering, streaming collection, and lifecycle abuse controls are prerequisites for sustainable gross margins.
	- Token theft is an important leading indicator because it reveals both the economic value of inference and the weakness of traditional payment-only fraud systems.
	- The most consequential 12-month development would be an agent that behaves as a small firm rather than a shopping assistant. That would transform payments from a human checkout function into autonomous economic infrastructure.
