...
- ## Stock Read-Throughs
	- **[[$NVDA]] — broader moat, broader obligations**
		- Bull case: owns the general GPU, scale-up fabric, networking, cache software, and increasingly specialized prefill/decode engines.
		- Bull case: rapid model change favors programmable hardware and a supplier able to optimize new architectures immediately.
		- Pressure: open inference engines weaken CUDA lock-in, hyperscaler ASICs have captive demand, and specialized silicon can beat a GPU on narrow economics.
		- Key test: whether platform expansion sustains premium margins without excessive financing, equity support, or cannibalization among product lines.
	- **[[$AMD]] — valuable second source, not a base-case share winner**
		- vLLM/SGLang support lowers adoption friction, and periodic hardware parity can win price-sensitive inference.
		- NVIDIA's faster roadmap and deeper system software keep sustained share gains difficult.
	- **[[$TSM]] and [[$ASML]] — neutral suppliers to a fragmenting architecture market**
		- GPUs, TPUs, Trainium, MTIA, Groq-style devices, and startups all require advanced manufacturing and packaging.
		- China localization is the long-run competitive risk, while current restrictions reinforce the Western/Taiwanese chokepoint.
	- **[[$CRWV]] and [[$ORCL]] — demand leverage plus financing leverage**
		- Scarce power and anchor contracts create strategic value.
		- Returns depend on utilization after the first tenant, hardware obsolescence, refinancing, and continued model progress.
	- **[[$MSFT]], [[$GOOGL]], [[$AMZN]], and [[$META]] — internal chips become strategic, not optional**
		- At hyperscaler scale, saving even a fraction of inference cost justifies custom silicon and software investment.
		- Internal chips reduce NVIDIA dependence but require large engineering teams and stable captive workloads.
	- **Power and electrical equipment — nearer-term scarcity winners**
		- Existing dispatchable generation, gas equipment, substations, transformers, switchgear, and permitted interconnections monetize years before most new nuclear projects.
		- The main risk is a capex pause caused by model stagnation, financing stress, or political opposition after suppliers expand capacity.

- ## Scenario Framework
	- | Scenario | Path | Market outcome | Relative beneficiaries |
	  |---|---|---|---|
	  | Heterogeneous NVIDIA platform wins | GPU, CPX, Groq, networking, and cache software cover most workloads | NVIDIA retains system pricing even as individual chips specialize | [[$NVDA]], foundry, HBM, networking, storage |
	  | Open inference commoditizes hardware | vLLM/SGLang make accelerators interchangeable and model releases slow | Price competition lowers NVIDIA margins and raises AMD/custom share | [[$AMD]], hyperscalers, customers, open software |
	  | Capex earns through | AI ARR exceeds Patel's \$100B case and agents penetrate software/office labor | Current builds reach high utilization after a lag | [[$NVDA]], [[$CRWV]], [[$ORCL]], power and electrical suppliers |
	  | Model-progress wall | More compute fails to create materially better systems | Research demand, financing confidence, and premium rents reverse | Software incumbents and less-levered infrastructure owners |
	  | China bifurcation | Export controls persist and Huawei's ecosystem compounds | Two software/hardware stacks emerge; Western vendors lose China but retain a lead | Huawei/Chinese supply chain domestically; Western chokepoints ex-China |
	  | Power-limited buildout | Chips are available but generation and interconnection lag | Projects cluster around gas, existing plants, and behind-the-meter sites | IPPs, turbines, engines, grid equipment, powered-land owners |

- ## Key Tests for the Thesis
	- **Model progress**: production capability and task completion from the next OpenAI, Anthropic, and Google releases—not benchmark marketing alone.
	- **AI revenue conversion**: whether industry ARR approaches the attributed \$100 billion 2026 scenario and supports depreciation plus financing cost.
	- **Inference share**: accelerator deployments by training, prefill, decode, video, and recommendation rather than one aggregate GPU number.
	- **NVIDIA platform attachment**: networking, CPUs, storage/cache software, and specialized engines per GPU system.
	- **AMD production share**: sustained usage and optimized time-to-first-token/cost-per-token on current models, not announced availability.
	- **Agent adoption**: GitHub commits, tokens per developer, task duration, non-coder usage, and enterprise willingness to reduce junior hiring.
	- **Power delivery**: energized megawatts, turbine/engine delivery, interconnections, and behind-the-meter contracts rather than planned gigawatts.
	- **China substitution**: Huawei accelerator output, HBM access, domestic tool progress, and whether open-source frameworks optimize first-class support for Chinese chips.
	- **Credit quality**: customer concentration, take-or-pay enforceability, residual asset value, and later-year funding for multi-year cluster leases.
